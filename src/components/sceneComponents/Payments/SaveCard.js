...
const saveCardEndPoint = `${BASE_URL}payment/subscription/`;
...
<div class="globalContent">
  ...
  <div class="cell example example2" id="example-2">
      ...
      <button type="submit" data-tid="elements_examples.form.pay_button">Link Your Card</button>
...
const SaveCard = (props) => {
    ...
    checkAuth((userInfo) => {
      if (userInfo && Object.keys(userInfo).length) {
        ...
        const injectedPayment = injectedJavaScript
          ...
          .replace('saveCardEndPoint', saveCardEndPoint)
  ...
  const handleResponse = (data) => {
    if (redirectUrl === data.url) {
      props.navigation.navigate('LinkedCards', {
        from: 'stripe',
      });
    }
  };
