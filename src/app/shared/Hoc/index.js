    ...
    switch (type) {
      ...
      case 'UPDATE_MISSION_PAYLOAD': {
        ...
        if (action.payload.key === 'milestones') {
          ...
          // ]);
          payloadData = orderBy(mileStoneCopy, (d) => +d.amount, ['asc']);
        }
        return {
          ...state,
          createMissionPayload: {
      ...
      case 'UPDATE_MILESTONE': {
        ...
        mileStoneCopy[action.payload.index] = action.payload.data;
        // const payloadData = orderBy(mileStoneCopy, [
        //   'amount',
        //   'percentage_completed',
        // ]);
        const payloadData = orderBy(mileStoneCopy, (d) => +d.amount, ['asc']);
        return {
          ...state,
          createMissionPayload: {
            ...state.createMissionPayload,
            milestones: payloadData,
          },
        };
      }
      case 'DELETE_MILESTONE': {
        const mileStoneCopy = [...state.createMissionPayload.milestones];
        // mileStoneCopy.splice(mileStoneCopy.length - 1, 1);
        mileStoneCopy.splice(action.payload.index, 1);
        return {
          ...state,
          createMissionPayload: {
            ...state.createMissionPayload,
      ...
      case 'SET_AFTER_LOGIN_ROUTE':
        return {
          ...state,
          afterLoginRoute: action.payload.afterLoginRoute,
          afterLoginParam: action.payload.afterLoginParam,
        };
      case 'LOGOUT':
