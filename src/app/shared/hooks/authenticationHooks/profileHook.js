...
export const useProfileHook = (
  ...
  useEffect(() => {
    if (userProfile && userProfile.id) getProfile(userProfile.id);
  }, []);
  useEffect(() => {
    ...
    if (
      userProfile &&
      userProfile.email &&
      userProfile.email.includes('@charitable')
    ) {
      setEmailAddress('');
    } else {
      setEmailAddress(firebaseProfile.email || userProfile.email);
    }
    if (firebaseProfile.email) {
      setEmailAddress(firebaseProfile.email);
    }
    ...
    if (userProfile && userProfile.google_id) {
      setIsEmailDisabled(true);
    }
      ...
      endPoint({
        ...
        callback: {
          ...
          errorCallback: ({
            ...
            if (errors && errors.phone && errors.phone.length) {
              setMobileNoError(errors.phone[0]);
            }
            if (errors && errors.email && errors.email.length) {
              setEmailAddressError(errors.email[0]);
            }
